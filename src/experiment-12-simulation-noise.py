...
p = np.linspace(-1.0, 1.0, num=n_buckets)
...
plt.figure()
ax = plt.plot(range(1, n_buckets + 1), y_mean, color="red")
plt.plot(range(1, n_buckets + 1), y_exact, color="green")

plt.violinplot(torch.transpose(y, 0, 1), vert=True)
